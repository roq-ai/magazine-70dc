...
export const getTranslations = async (query?: TranslationGetQueryInterface) => {
  const response = await axios.get(`/api/translations${query ? `?${queryString.stringify(query)}` : ''}`);
