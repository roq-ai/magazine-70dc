...
function ArticleListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ArticleInterface[]>(
    ...
    () =>
      getArticles({
        relations: ['category', 'user_article_author_idTouser', 'user_article_editor_idTouser', 'translation.count'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_article_author_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_article_author_idTouser?.id}`}>
                          {record.user_article_author_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    ...
                    {hasAccess('translation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.translation}</Td>
                    )}
                    <Td>
