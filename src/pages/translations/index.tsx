...
import { getTranslations, deleteTranslationById } from 'apiSdk/translations';
...
function TranslationListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('translation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/translations/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('translation', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Translation
            </Text>
            <NextLink href={`/translations/create`} passHref legacyBehavior>
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('article', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/articles/view/${record.article?.id}`}>
                    ...
                    <Td>
                      {hasAccess('translation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/translations/edit/${record.id}`} passHref legacyBehavior>
                          <Button
