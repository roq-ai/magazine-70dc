...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getOverviewPages() {
    const data = await prisma.overview_page
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'overview_page'));
  ...
  async function createOverviewPage() {
    ...
    const data = await prisma.overview_page.create({
      data: body,
    });
    return res.status(200).json(data);
  }
